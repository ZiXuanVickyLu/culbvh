...
void save_results_to_csv(const std::string& dataset_name, const std::vector<TestResult>& results) {
    ...
    std::string csv_path = output_dir + dataset_name + "_stackless_lite.csv";
...
TestResult test_frame(const std::string& dataset_name, int frame_num) {
    ...
    // Test culbvh with ground truth comparison
    {
        ...
        std::cout << "  Testing Stackless LBVH Lite..." << std::endl;
        
        // Build collider BVH
        cudaEventRecord(start);
        culbvh::LBVHStacklessLite collider_bvh;
        ...
        // Build mesh BVH
        cudaEventRecord(start);
        culbvh::LBVHStacklessLite mesh_bvh;
        ...
        cudaEventRecord(start);
        result.query_contacts = mesh_bvh.query(thrust::raw_pointer_cast(d_results.data()), max_results, thrust::raw_pointer_cast(d_collider_aabbs.data()), collider_aabbs.size());
        cudaEventRecord(stop);
        cudaEventSynchronize(stop);
        ...
        result.query_matches_ground_truth = mesh_bvh.query_compare_ground_truth(
            thrust::raw_pointer_cast(d_results.data()), result.query_contacts, thrust::raw_pointer_cast(d_collider_aabbs.data()), collider_aabbs.size());
...
int main() {
    std::cout << "CUDA Stackless LBVH Lite Simulation Dataset Test" << std::endl;
    std::cout << "==============================================" << std::endl;
