/*
 * Export the AABB list as a obj file (wireframe sets), which can be visualized by other software
 */
...
// Simple JSON parser for config.json (no external library needed)
struct Config {
...
int main() {
    std::cout << "CULBVH AABB Visualizer" << std::endl;
    std::cout << "======================" << std::endl;
    ...
    std::cout << "Generating AABB wireframe..." << std::endl;
    wireframe.build(device_aabbs);
    
    if (!wireframe.is_valid()) {
        std::cout << "Error: Failed to generate wireframe" << std::endl;
        return -1;
    }
    ...
    std::cout << std::endl;
    std::cout << "AABB visualization completed successfully!" << std::endl;
    std::cout << "Output file: " << output_path << config.getOutputName() << ".obj" << std::endl;
